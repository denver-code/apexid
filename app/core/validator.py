...
def check_scope(required_scopes, user, top_level=[]):
    if f"urn:zitadel:iam:org:project:{settings.PROJECT_ID}:roles" not in user:
        raise HTTPException(status_code=403, detail="Forbidden")

    user_roles = user[f"urn:zitadel:iam:org:project:{settings.PROJECT_ID}:roles"].keys()

    if top_level:
        for top_scope in top_level:
            if top_scope in user_roles:
                return
